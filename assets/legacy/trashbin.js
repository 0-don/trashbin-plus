...
(function TrashBin() {
  ...
  function settingsContent() {
    ...
    content.appendChild(
      createButton(
        "Copy",
        "Copy all items in trashbin to clipboard.",
        copyItems,
      ),
    );
    content.appendChild(
      createButton(
        "Export",
        "Save all items in trashbin to a .json file.",
        exportItems,
      ),
    );
    content.appendChild(
      createButton(
        "Import",
        "Overwrite all items in trashbin via .json file.",
        importItems,
      ),
    );
    content.appendChild(
      createButton(
        "Clear ",
        "Clear all items from trashbin (cannot be reverted).",
        () => {
          trashSongList = {};
          trashArtistList = {};
  ...
  setWidgetState(
    ...
    Spicetify.URI.fromString(Spicetify.Player.data.item.uri).type !==
  ...
  function watchChange() {
    ...
    setWidgetState(
      isBanned,
      Spicetify.URI.fromString(data.item.uri).type !== Spicetify.URI.Type.TRACK,
    );
    ...
    if (!list[uri]) {
      ...
      Spicetify.showNotification(
        type === Spicetify.URI.Type.TRACK
          ? "Song added to trashbin"
          : "Artist added to trashbin",
      );
    } else {
      delete list[uri];
      ...
      Spicetify.showNotification(
        type === Spicetify.URI.Type.TRACK
          ? "Song removed from trashbin"
          : "Artist removed from trashbin",
      );
    }
